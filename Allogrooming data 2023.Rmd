...
v2022_names <- v2022_names %>% mutate(bat_name_full = paste( sep = "_", source, paste(bat_name, "22", sep="")))
view(v2022_names)
...
obs_pair_all_m <- v2022_allogr %>%
      mutate(actor_name = paste(group, actor_name, sep = "_"), 
             receiver
      filter(behavior == "m") %>%
      group_by(actor_name,receiver_name) %>%  # or : v2022_allogr %>% count(actor,receiver )
      summarise(n = n()) %>% 
      arrange(-n) %>%
      group_by(group) %>%
